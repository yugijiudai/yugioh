...
public class MyBeanUtil {
    ...
    public static <T> List<T> convertListMap2ListBean2(List<? extends Model> listMap, Class T) throws Exception {
        List<T> beanList = new ArrayList<>();
        for (int i = 0, n = listMap.size(); i < n; i++) {
            String json = listMap.get(0).toJson();
            T aClass = new Gson().fromJson(json, (Class<T>) T);
